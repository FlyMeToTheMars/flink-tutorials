package io.github.streaming.examples.flink.triggers;
...
public class CountTriggerWithTimeout<T> extends Trigger<T, TimeWindow> {
  ...
  public TriggerResult onElement(T element, long timestamp, TimeWindow window, TriggerContext ctx)
    ...
    if (timestamp >= window.getEnd()) {
      if (LOG.isDebugEnabled()) {
        LOG.debug("fire with time: " + timestamp);
      }
      return fireAndPurge(window, ctx);
    } else {
  ...
  public TriggerResult onProcessingTime(long time, TimeWindow window, TriggerContext ctx)
    ...
    if (time >= window.getEnd()) {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug("fire with process time: " + time);
      }
      return fireAndPurge(window, ctx);
    }
  }
  ...
  public TriggerResult onEventTime(long time, TimeWindow window, TriggerContext ctx)
    ...
    if (time >= window.getEnd()) {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug("fire with event time: " + time);
      }
      return fireAndPurge(window, ctx);
    }
  }
